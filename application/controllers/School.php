...
class School extends CI_Controller
{
	...
	function student_bulk_add($param1 = '')
	{
		...
		if($param1 == 'add_bulk_student') {
			...
			redirect(base_url() . 'school/student_information/' . $this->input->post('class_id') , 'refresh');
		}			
...
function products($product_category_id)
    ...
    function product_list($product_category_id)
	{
		...
		if ($product_category_id == NULL) {
            redirect(base_url() . 'seller/product_list/1', 'refresh');
	    ...
	    function rankcalculate($class_id = '' , $section_id = '')
    {
    	...
    	redirect(base_url() . 'school/student_information/' . $class_id , 'refresh');
    }
	...
	function student_marksheet_print_view($student_id , $exam_id) {
		...
		$templete = $this->db->get_where('school_setting',array('school_id'=>$this->session->userdata('school_id')))->row_array();
		$color  =$templete['marksheet_templete'];
		if(!$templete){
		    $color = '1';
		}
		$page_name = $this->db->get_where('marksheet_templete',array('templete_id'=>$color))->row()->page_name;
		$class_name   = $this->db->get_where('class' , array('class_id' => $class_id))->row()->name;
		$page_data['school_id'] = $this->session->userdata('school_id');
		$page_data['student_id'] =   $student_id;
		$page_data['class_id']   =   $class_id;
		$page_data['exam_id']    =   $exam_id;
    	$this->load->view('backend/school/'.$page_name, $page_data);
	}
...
function gamecerti($student_id , $exam_id) {
	...
	function student($param1 = '', $param2 = '', $param3 = '')
	{
		...
		if ($param1 == 'create') {
			...
			redirect(base_url() . 'school/student_add/', 'refresh');
		}
		if ($param1 == 'do_update') {
			$data['name']           = $this->input->post('name');
			$data['birthday']       = strtotime(str_replace('/', '-', $this->input->post('birthday')));
			...
			redirect(base_url() . 'school/student_information/' . $param3, 'refresh');
		} 
		...
		if ($param2 == 'delete') {
			...
			redirect(base_url() . 'school/student_information/' . $param1, 'refresh');
		}
	}
	...
	function student_promotion($param1 = '' , $param2 = '')
	{
		...
		if($param1 == 'promote') {
			...
			redirect(base_url() . 'school/student_promotion' , 'refresh');
		}
	...
	function parent($param1 = '', $param2 = '', $param3 = '')
	{
		...
		if ($param1 == 'create') {
            ...
            redirect(base_url() . 'school/parent/', 'refresh');
        }
        if ($param1 == 'edit') {
        	$data['name']                   = $this->input->post('name');
        	$data['email']                  = $this->input->post('email');
...
//echo "<script>alert('working');</script>";
    ...
    function teacher($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'create') {
            ...
            redirect(base_url() . 'school/teacher/', 'refresh');
        }
        if ($param1 == 'do_update') {
        	$data['name']        = $this->input->post('name');
    		$data['birthday']    = strtotime(str_replace('/', '-', $this->input->post('birthday')));
...
//data['school_id'] = $this->session->userdata('school_id');
    ...
    function subject($param1 = '', $param2 = '' , $param3 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/subject/'.$data['class_id'], 'refresh');
    	}
    	if ($param1 == 'do_update') {
    		$data['name']       = $this->input->post('name');
    		$data['class_id']   = $this->input->post('class_id');
    	...
    	if ($param1 == 'delete') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
    		redirect(base_url() . 'school/subject/'.$param3, 'refresh');
    	}
    ...
    function classes($param1 = '', $param2 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/classes/', 'refresh');
    	}
    	if ($param1 == 'do_update') {
    		$data['name']         = $this->input->post('name');
    		$data['school_id'] = $this->session->userdata('school_id');
...
function get_by_category($category) 
    {	
    	...
    	foreach ($expense as $key) {
    		$url = base_url() . 'school/more_expense_info/' . $key['payment_id'];
    		echo '<tr><td>' . $count . '</td>';
...
function get_by_date($date) 
    { 	
    	...
    	$totalamount = 0;
    	foreach ($expense as $key) {
    		$url = base_url() . 'school/more_info/' . $key['student_id'];
    		echo '<tr><td>' . $count . '</td>';
    		echo '<td>' . $key['title'] . '</td>';
    		echo '<td>' . $key['description'] . '</td>';
    		$totalamount =$totalamount+ $key['amount'];
...
//get info by SR number
 ...
 function more_info($student_id)
	...
	$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    	redirect(base_url() . 'school/teacher_salary/', 'refresh');
    ...
    {	
    	...
    	redirect(base_url() . 'school/academic_syllabus/' . $data['class_id'] , 'refresh');
    ...
    function sections($param1 = '' , $param2 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/section/' . $data['class_id'] , 'refresh');
    	}
    	...
    	if ($param1 == 'delete') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
    		redirect(base_url() . 'school/section' , 'refresh');
    	}
    }
    ...
    function exam($param1 = '', $param2 = '' , $param3 = '')
    {
        ...
        //$data['comment'] = $this->input->post('comment');
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/exam/', 'refresh');
    	}
    ...
    function exam_marks_sms($param1 = '' , $param2 = '')
    {
    	...
    	if ($param1 == 'send_sms') {
    		...
    		redirect(base_url() . 'school/exam_marks_sms' , 'refresh');
    	}
    ...
    function marks2($exam_id = '', $class_id = '', $subject_id = '')
    {
    	...
    	if ($this->input->post('operation') == 'selection') {
    		...
    		if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['subject_id'] > 0) {
    			redirect(base_url() . 'school/marks2/' . $page_data['exam_id'] . '/' . $page_data['class_id'] . '/' . $page_data['subject_id'], 'refresh');
    		} else {
    			$this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
    			redirect(base_url() . 'school/marks2/', 'refresh');
    		}
    	}
    	if ($this->input->post('operation') == 'update') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
    		redirect(base_url() . 'school/marks2/' . $this->input->post('exam_id') . '/' . $this->input->post('class_id') . '/' . $this->input->post('subject_id'), 'refresh');
    	}
    	$page_data['exam_id']    = $exam_id;
    	$page_data['class_id']   = $class_id;
    ...
    function marks_manage_view($exam_id = '' , $class_id = '' , $section_id = '' , $subject_id = '')
    {
    	...
    	$page_data['runnug_year'] = $this->db->get_where('school',array('school_id'=>$this->session->userdata('school_id')))->row()->running_year;
    ...
    function marks_selector()
    {
    	...
    	redirect(base_url() . 'school/marks_manage_view/' . $data['exam_id'] . '/' . $data['class_id'] . '/' . $data['section_id'] . '/' . $data['subject_id'] , 'refresh');
    ...
    function marks_update($exam_id = '' , $class_id = '' , $section_id = '' , $subject_id = '')
    {
    	...
    	redirect(base_url().'school/marks_manage_view/'.$exam_id.'/'.$class_id.'/'.$section_id.'/'.$subject_id , 'refresh');
    }
...
function insert_new_student($class_id = '',$section_id = '' , $student_id = '')
    ...
    function tabulation_sheet($class_id = '', $section_id , $exam_id = '') {
    	...
    	if ($this->input->post('operation') == 'selection') {
    		...
    		if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0) {
    			redirect(base_url() . 'school/tabulation_sheet/' . $page_data['class_id'] . '/'. $page_data['section_id'] . '/'. $page_data['exam_id'] , 'refresh');
    		} else {
    			$this->session->set_flashdata('mark_message', 'Choose class and exam');
    			redirect(base_url() . 'school/tabulation_sheet/', 'refresh');
    		}
    	}
    	$page_data['exam_id']    = $exam_id;
    	$page_data['class_id']   = $class_id;
    ...
    function marksheet_templetes(){
        	if ($this->session->userdata('school_login') != 1)
    		redirect(base_url(), 'refresh');
    		$page_data['page_name']  = 'marksheet_templetes';
    	    $page_data['page_title'] = get_phrase('Select_Marksheet_Templete');
    	    $this->load->view('backend/index', $page_data);
    }
    
    function select_marksheet_templete($templete_id = ''){
        if ($this->session->userdata('school_login') != 1)
    		redirect(base_url(), 'refresh');
    		
    	$school_id = $this->session->userdata('school_id');
    	$check = $this->db->get_where('school_setting',array('school_id'=>$school_id));
    	if($check->num_rows() > 0){
    	    $data['marksheet_templete'] = $templete_id;
    	    $data['time'] = time();
    	    $this->db->where('school_id',$school_id);
    	    $this->db->update('school_setting',$data);
    	    $this->session->set_flashdata('flash_message' , get_phrase('Templete_Added_Successfully'));
    		redirect(base_url() . 'school/marksheet_templetes/', 'refresh');
    	}else{
    	    $data['school_id'] = $school_id;
    	    $data['marksheet_templete'] = $templete_id;
    	    $data['time'] = time();
    	    $this->db->insert('school_setting',$data);
    	    $this->session->set_flashdata('flash_message' , get_phrase('Templete_Added_Successfully'));
    		redirect(base_url() . 'school/marksheet_templetes/', 'refresh');
    	}
    ...
    function grade($param1 = '', $param2 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/grade/', 'refresh');
    	}
    	if ($param1 == 'do_update') {
    		$data['name']        = $this->input->post('name');
    		$data['grade_point'] = $this->input->post('grade_point');
    ...
    function class_routine($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/class_routine_add/', 'refresh');
    	}
    	if ($param1 == 'do_update') {
    		$data['class_id']       = $this->input->post('class_id');
    		if($this->input->post('section_id') != '') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
    		redirect(base_url() . 'school/class_routine_view/' . $data['class_id'], 'refresh');
    	} else if ($param1 == 'edit') {
    	...
    	if ($param1 == 'delete') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
    		redirect(base_url() . 'school/class_routine_view/' . $class_id, 'refresh');
    	}
    ...
    function attendance_selector()
    {
    	...
    	redirect(base_url().'school/manage_attendance_view/'.$data['class_id'].'/'.$data['section_id'].'/'.$data['timestamp'],'refresh');
    }
    ...
    function attendance_update($class_id = '' , $section_id = '' , $timestamp = '')
    {
    	...
    	redirect(base_url().'school/manage_attendance_view/'.$class_id.'/'.$section_id.'/'.$timestamp , 'refresh');
    }
    ...
    function manage_attendance2($date='',$month='',$year='',$class_id='' , $section_id = '' , $session = '')
    {
    	...
    	if($_POST)
    	{
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
    		redirect(base_url() . 'school/manage_attendance/'.$date.'/'.$month.'/'.$year.'/'.$class_id.'/'.$section_id.'/'.$session , 'refresh');
    	}
    	$page_data['date']       =	$date;
    ...
    function attendance_selector2()
    {
    	...
    	redirect(base_url() . 'school/manage_attendance/'.$this->input->post('date').'/'.
    ...
    function attendance_report_selector()
    {
    	...
    	$data['section_id'] = $this->input->post('section_id');
    	redirect(base_url().'school/attendance_report_view/'.$data['class_id'].'/'.$data['section_id'].'/'.$data['month'].'/'.$data['selectyear'],'refresh');
    }
    ...
    function invoice($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'delete') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
    		redirect(base_url() . 'school/income', 'refresh');
    	}
    ...
    function expense($param1 = '' , $param2 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->db->insert('payment' , $data);
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/expense', 'refresh');
    	}
    ...
    function expense_category($param1 = '' , $param2 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/expense_category');
    	}
    	if ($param1 == 'edit') {
    		$data['name']   =   $this->input->post('name');
    ...
    function book($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/book', 'refresh');
    	}
    	if ($param1 == 'do_update') {
    		$data['name']        = $this->input->post('name');
    		$data['description'] = $this->input->post('description');
    ...
    function transport($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/transport', 'refresh');
    	}
    	if ($param1 == 'do_update') {
    		$data['route_name']        = $this->input->post('route_name');
    		$data['number_of_vehicle'] = $this->input->post('number_of_vehicle');
    ...
    function dormitory($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/dormitory', 'refresh');
    	}
    	if ($param1 == 'do_update') {
    		$data['name']           = $this->input->post('name');
    		$data['number_of_room'] = $this->input->post('number_of_room');
    ...
    function noticeboard($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'create') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
    		redirect(base_url() . 'school/noticeboard/', 'refresh');
    	}
    	if ($param1 == 'do_update') {
    		$data['notice_title']     = $this->input->post('notice_title');
    		$data['notice']           = $this->input->post('notice');
    ...
    function message($param1 = '', $param2 = '', $param3 = '') {
    	...
    	if ($param1 == 'single') {
    		...
    		$check = $this->db->get_where('user_token',array('user_id'=>$student_id,'user_type'=>'Student'));
    		if($check->num_rows() > 0){
    		    $to = $check->row()->device_token;
    		    $title = $this->db->get_where('school',array('school_id'=>$this->session->userdata('school_id')))->row()->school_name;
    		    ...
    		    $data['other_type']  ='none';
    		    $data['time'] = time();
    		    $this->db->insert('firebase_notification',$data);
        ...
        if ($param1 == 'bulk_sms') {
            ...
            $class_id = $this->input->post('class_id');
            $running_year = $this->db->get_where('school',array('school_id'=>$this->session->userdata('school_id')))->row()->running_year;
            $student_ids = $this->db->get_where('enroll',array('class_id'=>$class_id,'year'=>$running_year))->result_array();
            $registration_ids = array();
            $message = $this->input->post('class_message');
            $title = $this->db->get_where('school',array('school_id'=>$this->session->userdata('school_id')))->row()->school_name;
            foreach($student_ids as $row){
                $check = $this->db->get_where('user_token',array('user_id'=>$row['student_id'],'user_type'=>'Student'));
                if($check->num_rows() > 0){
        ...
        if ($param1 == 'whole_sms') {
            $tokens = $this->db->get_where('user_token',array('school_id'=>$this->session->userdata('school_id')))->result_array();
            $registration_ids = array();
            $title = $this->db->get_where('school',array('school_id'=>$this->session->userdata('school_id')))->row()->school_name;
    ...
    function system_settings($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'do_update') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_updated')); 
    		redirect(base_url() . 'school/system_settings/', 'refresh');
    	}
    ...
    function update( $task = '', $purchase_code = '' ) {
    	...
    	redirect(base_url() . 'school/system_settings');
    }
    ...
    function sms_settings($param1 = '' , $param2 = '')
    {
    	...
    	if ($param1 == 'clickatell') {
    		...
    		$this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
    		redirect(base_url() . 'school/sms_settings/', 'refresh');
    	}
    ...
    function manage_language($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'update_phrase') {
    		...
    		redirect(base_url() . 'school/manage_language/edit_phrase/'.$language, 'refresh');
    	}
    	if ($param1 == 'do_update') {
    		...
    		$this->session->set_flashdata('flash_message', get_phrase('settings_updated'));
    		redirect(base_url() . 'school/manage_language/', 'refresh');
    	}
    ...
    function backup_restore($operation = '', $type = '')
    {
    	...
    	if ($operation == 'restore') {
    		...
    		redirect(base_url() . 'school/backup_restore/', 'refresh');
    	}
    	if ($operation == 'delete') {
    ...
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'update_profile_info') {
    		$data['school_username']  = $this->input->post('name');
    		$data['email'] = $this->input->post('email');
    		...
    		redirect(base_url() . 'school/manage_profile/', 'refresh');
    	}
    ...
    function librarian($param1 = '', $param2 = '', $param3 = '')
    {
    	...
    	if ($param1 == 'create') {
            ...
            redirect(base_url() . 'school/librarian/', 'refresh');
        }
