...
class Mobile extends CI_Controller
{
    ...
    function get_class() {
        $response      =   array();
        $class       = array();
        $classes       =   $this->db->get('vid_class')->result_array();
        foreach($classes as $row) {
            $data['class_id']     =   $row['vid_cls_id'];
            $data['name']         =   $row['name'];
            $data['image']        =   base_url().'uploads/classes/'.$row['image'];

            array_push($class , $data);

        }
        if($class){
            $response = array('status'=>'1','msg'=>'All classes are here','data'=>$class);
        }else{
            $response = array('status'=>'0','msg'=>'Sorry no classes');
        }
    ...
    function get_subject() {
        ...
        $response      =   array();
        $sub = array();
        $subjects       =   $this->db->get_where('vid_subject',array('class_id'=>$class_id))->result_array();
        foreach($subjects as $row) {
            $data['subject_id']     =   $row['vid_subject_id'];
            $data['name']         =   $row['name'];
            $data['image']        =   base_url().'uploads/subjects/'.$row['image'];
  
            array_push($sub , $data);
        }
        if($sub){
            $response = array('status'=>'1','msg'=>'All Subject are here','data'=>$sub);
        }else{
            $response = array('status'=>'0','msg'=>'Sorry no Subject in this Class');
        }
        echo json_encode($response);
    }
    ...
    function get_topics() {
        ...
        $response      =   array();
        $sub = array();
        $topics       =   $this->db->get_where('topics',array('course_id'=>$subject_id))->result_array();
        foreach($topics as $row) {
            $data['topic_id']     =   $row['topic_id'];
            $data['name']         =   $row['name'];
            $data['image']        =   base_url().'uploads/topics/'.$row['image'];
  
            array_push($sub , $data);

        }
        if($sub){
            $response = array('status'=>'1','msg'=>'All Topics are here','data'=>$sub);
        }else{
            $response = array('status'=>'0','msg'=>'Sorry no Topic in this Subject');
        }
        echo json_encode($response);
    }
    ...
    function videos(){
        ...
        $videos = $this->db->get_where('videos',array('topic_id'=>$topic_id))->result_array();
        $out = array();
        foreach($videos as $row){
            $row['file'] = base_url().'uploads/videos/'.$row['file'];
            $row['thumbnail'] = base_url().'uploads/thumbnails/'.$row['thumbnail'];
            array_push($out,$row);
        }
        if($videos){
            echo json_encode(array('status'=>'1','message'=>'video are available','Data'=>$out));
        }else{
            echo json_encode(array('status'=>'0','message'=>'there no videos available in this course'));
        }
    }
    ...
    function get_system_info() {
        ...
        $response['contact_mail']    =   $this->db->get_where('settings' , array('type' => 'contact_mail'))->row()->description;
        $response['school_contact_no']    =   $this->db->get_where('settings' , array('type' => 'school_contact_no'))->row()->description;
        echo json_encode($response);
    }
