...
class Login extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->session->userdata('librarian_login') == 1)
            redirect(base_url() . 'index.php?librarian/dashboard', 'refresh');
             ...
             redirect(base_url() . 'index.php?introsys/index', 'refresh');
        }
    ...
    function validate_login($email = '', $password = '') {
        ...
        // Checking login credential for teacher
        $query = $this->db->get_where('seller', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('seller_login', '1');
            $this->session->set_userdata('seller_id', $row->seller_id);
            $this->session->set_userdata('login_user_id', $row->seller_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'seller');
            return 'success';
        }
        // Checking login credential for teacher
