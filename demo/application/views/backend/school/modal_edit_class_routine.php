...
<div class="tab-pane box active" id="edit" style="padding: 5px">
    <div class="box-content">
        ...
        <?php echo form_open(base_url() . 'index.php?school/class_routine/do_update/'.$row['class_routine_id'] , array('class' => 'form-horizontal validatable','target'=>'_top'));?>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-5">
                        <select id="subject_id" name="subject_id" class="form-control selectboxit" onchange="section_subject_select(this.value , <?php echo $param2;?>)">
                            <?php 
                            $subjects = $this->db->get_where('subject', array('class_id' => $row['class_id']))->result_array();
                            foreach($subjects as $row2):
                            ?>
                                <option value="<?php echo $row2['subject_id'];?>" <?php if($row['subject_id']==$row2['subject_idsa'])echo 'selected';?>>
                                    <?php echo $row2['name'];?></option>
                            <?php
                            endforeach;
                            ?>
...
<script type="text/javascript">
    function section_subject_select(class_id , class_routine_id) {
        $.ajax({
            url: '<?php echo base_url();?>index.php?school/section_subject_edit/' + class_id + '/' + class_routine_id ,
